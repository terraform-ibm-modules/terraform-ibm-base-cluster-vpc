##############################################################################
# base-cluster-vpc-module
# Deploy IKS or OpenShift cluster in IBM Cloud on VPC Gen 2
##############################################################################
...
resource "ibm_container_vpc_cluster" "cluster" {
  ...
  secondary_storage                   = local.is_ocp ? local.default_pool.secondary_storage : null
  ...
  kms_account_id                      = local.is_ocp ? (local.default_pool.boot_volume_encryption_kms_config == null ? null : local.default_pool.boot_volume_encryption_kms_config.kms_account_id) : null
...
resource "kubernetes_config_map_v1_data" "set_autoscaling" {
  ...
  data = merge(
    # OCP supports additional cluster_autoscaler_config keys; IKS only uses workerPoolsConfig.json
    local.is_ocp ? {
...
##############################################################################
# Attach additional security groups to VPEs managed by this cluster.
# https://cloud.ibm.com/docs/openshift?topic=openshift-vpc-security-group&interface=ui#vpc-sg-vpe-alb
##############################################################################
...
resource "time_sleep" "wait_for_auth_policy" {
  count           = var.enable_secrets_manager_integration ? 1 : 0
  depends_on      = [ibm_iam_authorization_policy.secrets_manager_iam_auth_policy[0]]
  create_duration = "30s"
}
